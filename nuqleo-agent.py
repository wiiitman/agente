...
def _sync_custom_modules() -> bool:
    ...
    tmp = MODULES_DIR + '.tmp'
    run(f'rm -rf {tmp}')
    r = run(f'git clone --depth 1 {auth_url} {tmp}', timeout=180)
    if r['ok']:
        run(f'rm -rf {MODULES_DIR} && mv {tmp} {MODULES_DIR}')
        return True
    run(f'rm -rf {tmp}')
    log(f"[modules] sync FAIL: {r['stderr'][:150]}")
    return False
...
"""
    ...
    if not os.path.exists(cert_path):
        return {'ok': False,
                'error': 'No se pudo emitir el certificado SSL. Verifica que el DNS del dominio apunte a este servidor.',
                'detail': r['stderr'][:300]}
...
"""
    ...
    run(['systemctl', 'reload', 'nginx'])
    return {'ok': True, 'domain': domain, 'ssl': True, 'message': f'SSL activo para {domain}'}
...
class NuqleoHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/ping':
            ...
            _sync_custom_modules()  # best-effort: trae lo último del repo
    ...
    def _handle_deploy(self, body: dict):
        ...
        # Publicar en 0.0.0.0 para que el cliente acceda desde su navegador.
...
services:
  {container}:
    ...
    command: -- --db-filter=^{db_name}$$ --no-database-list
    volumes:
...
"""
        ...
        def _do_deploy():
            ...
            _set_stage(container, 'Preparando template de base de datos...')
            _ensure_template_db(version)
            _set_stage(container, 'Creando base de datos desde template...')
            ...
            # 3. Pre-pull de la imagen para que compose arranque sin esperar descarga
            _set_stage(container, 'Verificando imagen Docker...')
            run(f'docker pull odoo:{version}', timeout=300)

            # 3.5 Crear la carpeta de datos del volumen.
            odoo_data = os.path.join(deploy_dir, 'odoo-data')
            os.makedirs(odoo_data, exist_ok=True)

            # 3.6 Instalar los módulos seleccionados (+ el módulo custom si vino con
            # archivos) en la BD del cliente ANTES de servir. Se hace en un contenedor
            # one-off --stop-after-init para no chocar con el Odoo que sirve. Sin esto,
            # seleccionar "Sitio web" no dejaría el módulo website activo.
            # Traer la librería de módulos custom del repo y copiar la carpeta de ESTA
            # versión a los addons del deploy. Así --init puede instalar los custom
            # seleccionados y el contenedor los ve (ambos montan addons_dir).
            if MODULES_REPO:
                _sync_custom_modules()
                src = os.path.join(MODULES_DIR, str(version))
                if os.path.isdir(src):
                    run(f'cp -r {src}/. {addons_dir}/')
                    log(f'[deploy] {container}: módulos custom v{version} copiados a addons')

            mod_list = [m for m in modules.split(',') if m]
            if files:
                mod_list.append(module)
            mods_csv = ','.join(dict.fromkeys(mod_list))  # dedupe, conserva orden
            if mods_csv:
                _set_stage(container, f'Instalando módulos: {mods_csv}...')
                # NO montar odoo_data aquí: lo crea Docker como root y el one-off corre
                # como uid 100 → PermissionError y falla en segundos. El filestore se
                # regenera al servir. Solo montamos addons (para el módulo custom).
                # Flags largos como el init del template (forma probada que sí funciona).
                inst = run(
                ...
                ok_inst = inst['ok'] or 'stop' in (inst['stdout'] + inst['stderr']).lower()
                log(f"[deploy] {container}: install [{mods_csv}] ok={ok_inst} {inst['stderr'][:200]}")
                if not ok_inst:
                    # No bloquea: el Odoo arranca igual (con base); el cliente puede
                    # instalar a mano. Pero lo dejamos registrado.
                    _set_stage(container, f'Aviso: algún módulo no instaló ({mods_csv})')

            # 4. Levantar Odoo (arranca rápido porque la BD ya está inicializada)
            _set_stage(container, 'Iniciando contenedor Odoo...')
            r = run(f'cd {deploy_dir} && {COMPOSE} up -d', timeout=120)
            if r['ok']:
                # Odoo corre como un usuario no-root dentro del contenedor y necesita
                # escribir en /var/lib/odoo (sessions, filestore). Si el bind-mount del
                # host queda con dueño root → PermissionError → HTTP 500. Detectamos el
                # uid/gid real de la imagen y cedemos el volumen, luego reiniciamos para
                # que Odoo arranque limpio con los permisos correctos.
                uid_r = run(f'docker exec {container} id -u')
                gid_r = run(f'docker exec {container} id -g')
                uid = (uid_r['stdout'].strip() if uid_r['ok'] else '') or '100'
                gid = (gid_r['stdout'].strip() if gid_r['ok'] else '') or '101'
                run(f'chown -R {uid}:{gid} {odoo_data} {addons_dir}')
                run(f'docker restart {container}', timeout=60)
                _set_stage(container, 'Contenedor iniciado — Odoo cargando modulos...')

                # Reverse proxy con HTTP/2 → render fiable pese al packet loss (una
                # sola conexión multiplexada en vez de 6-8 que se caen).
                if subdomain:
                    # Subdominio con Let's Encrypt (requiere red fiable para validar).
                    _set_stage(container, f'Configurando {subdomain} con SSL...')
                    res = configure_domain_ssl(subdomain, port)
                    if res.get('ok'):
                        _set_stage(container, f'Listo — https://{subdomain}')
                    else:
                        _set_stage(container, f'Odoo listo (SSL pendiente: {(res.get("error") or "")[:80]})')
                elif ssl_mode == 'selfsigned' and PORT_MIN <= https_port <= PORT_MAX:
                    # HTTPS+HTTP2 con cert autofirmado (sin Let's Encrypt → funciona aun
                    # con la red mala, solo muestra aviso de "no seguro" en el navegador).
                    _set_stage(container, 'Configurando HTTPS (HTTP/2)...')
