...
"""
    ...
    def _handle_configure_domain(self, body: dict):
        ...
        # 1) Asegurar nginx + certbot instalados (idempotente / auto-reparable:
        #    aunque el VPS no los tuviera, este handler los instala al vuelo).
        ...
        #    Así el sitio responde por HTTP incluso antes de tener el certificado.
        http_conf = f"""server {{
    listen 80;
    server_name {domain};
...
"""
        ...
        # El vhost default captura cualquier server_name → quitarlo para no interferir.
        run('rm -f /etc/nginx/sites-enabled/default')
        test = run(['nginx', '-t'])
        if not test['ok']:
            return self._send(500, {'error': 'Error Nginx (http)', 'detail': test['stderr'][:300]})
        run(['systemctl', 'reload', 'nginx'])

        # 3) Emitir certificado por webroot (el método más fiable para automatizar:
        #    no edita nginx, solo necesita que el DNS ya apunte a este servidor).
        ...
        if not os.path.exists(cert_path):
            return self._send(502, {
                'error': 'No se pudo emitir el certificado SSL. Verifica que el DNS (registro A) del dominio ya apunte a la IP de este servidor y reintenta en unos minutos.',
                'detail': r['stderr'][:300],
            })

        # 4) Vhost final: 80 redirige a 443; 443 proxya al contenedor con SSL.
        full_conf = f"""server {{
    listen 80;
    server_name {domain};
    location /.well-known/acme-challenge/ {{ root {webroot}; }}
...
"""
        ...
        if test2['ok']:
            run(['systemctl', 'reload', 'nginx'])
            self._send(200, {'ok': True, 'domain': domain, 'ssl': True, 'message': f'SSL activo para {domain}'})
        else:
            self._send(500, {'error': 'Error Nginx (ssl)', 'detail': test2['stderr'][:300]})
