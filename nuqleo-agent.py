...
_pg_lock = threading.Lock()  # evitar condiciones de carrera al crear templates
...
def _pg_exec(sql: str) -> dict:
    return run(['docker', 'exec', SHARED_PG_NAME, 'psql', '-U', SHARED_PG_USER, '-c', sql])

def _pg_query(sql: str) -> str:
    r = run(['docker', 'exec', SHARED_PG_NAME, 'psql', '-U', SHARED_PG_USER, '-tAc', sql])
...
def _ensure_template_db(version: str) -> bool:
    ...
    with _pg_lock:
        ...
        _pg_exec(f'CREATE DATABASE {name}')
...
def _create_db_from_template(db_name: str, db_user: str, db_pass: str, version: str) -> bool:
    ...
    _pg_exec(f"CREATE USER {db_user} WITH PASSWORD '{db_pass}'")
