    ...
    try:
        ...
        if lang and lang != 'en_US':
            try:
                ...
                # Establecer idioma por defecto para el usuario admin
                admin_ids = models.execute_kw(db_name, uid, 'admin', 'res.users', 'search', [[['login', '=', 'admin']]])
                if admin_ids:
                    models.execute_kw(db_name, uid, 'admin', 'res.users', 'write', [admin_ids, {'lang': lang}])
                log(f'[rpc] {container}: idioma {lang} activado')
            except Exception as le:
                log(f'[rpc] {container}: idioma {lang} no activado: {le}')
...
"""
        ...
        def _do_deploy():
            ...
            # Detectar uid/gid real del contenedor y ceder los volúmenes.
            # Odoo corre como usuario no-root (uid varía según versión de imagen);
            # si el bind-mount queda como root → PermissionError → HTTP 500 en arranque.
            # Un restart rápido tras el chown es la forma más segura (10-15s, mucho
            # menos que la instalación de módulos que ya va en background).
            uid = (run(f'docker exec {container} id -u', timeout=8)['stdout'].strip()) or '101'
            gid = (run(f'docker exec {container} id -g', timeout=8)['stdout'].strip()) or '101'
