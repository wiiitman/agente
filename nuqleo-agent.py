...
"""
        ...
        r = run(f'cd {deploy_dir} && docker compose up -d', timeout=300)
        log(f"[deploy] {container}: {r['stderr'] or r['stdout']}")

        if not r['ok']:
            return self._send(500, {'error': 'Error al iniciar Docker', 'detail': r['stderr']})
        ...
        self._send(200, {
            ...
            'message':    f'Odoo {version} desplegado en puerto {port}',
        })
