...
# Cron para reintentear Let's Encrypt cada 1 hora (para dominios en fallback autofirmado)
# certbot renew intenta renovar certs que están próximos a vencer o nunca se emitieron.
echo "0 * * * * /usr/bin/certbot renew --quiet 2>&1 | logger -t nuqleo-certbot" | crontab -
echo "▶ Cron para certbot configurado (retry cada hora)"
