...
docker exec nuqleo_postgres_shared psql -U postgres -c "CREATE DATABASE odoo17_template OWNER odoo" 2>/dev/null || true
docker run --rm --network nuqleo-net \
  -e HOST=nuqleo_postgres_shared -e USER=odoo -e PASSWORD=nuqleo_pg_2024 \
  odoo:17 -- --database odoo17_template --init base --stop-after-init --no-http
docker exec nuqleo_postgres_shared psql -U postgres -c \
...
docker exec nuqleo_postgres_shared psql -U postgres -c "CREATE DATABASE odoo18_template OWNER odoo" 2>/dev/null || true
docker run --rm --network nuqleo-net \
  -e HOST=nuqleo_postgres_shared -e USER=odoo -e PASSWORD=nuqleo_pg_2024 \
  odoo:18 -- --database odoo18_template --init base --stop-after-init --no-http
docker exec nuqleo_postgres_shared psql -U postgres -c \
